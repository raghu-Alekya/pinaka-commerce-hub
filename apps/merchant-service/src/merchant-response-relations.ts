...
export class MerchantResponseRelations {
  ...
  async expand(payload: unknown): Promise<unknown> {
    ...
    if (wanted.subscription.size) {
      ...
      for (const row of rows) {records.subscription.set(key(row.id),row);collectRow(row);}
    }
    if (wanted.merchant.size) {
      const rows = await this.db.query(`SELECT requested.ref AS lookup,m.* FROM unnest($1::text[]) requested(ref)
        CROSS JOIN LATERAL (
          SELECT mer.*,to_char(mer."startDate",'YYYY-MM-DD') AS "startDate",to_char(mer."renewalDate",'YYYY-MM-DD') AS "renewalDate"
          FROM public.merchants mer LEFT JOIN public.merchant_record_versions v ON v.record_code=mer."merchantCode"
          WHERE mer."merchantId"=requested.ref OR mer."merchantCode"=requested.ref OR mer.id::text=requested.ref
          ORDER BY (mer.id::text=requested.ref OR mer."merchantCode"=requested.ref) DESC,
            (mer.status='ACTIVE') DESC,v.version DESC NULLS LAST,mer."createdAt" DESC,mer.id DESC LIMIT 1
        ) m`,[[...wanted.merchant]]);
    ...
    const visit = (value: unknown): unknown => {
      ...
      for (const kind of kinds) {
        ...
        // A merchant record already IS its merchant detail; never add a self-copy.
        ...
        // Preserve historical snapshots and existing computed response fields.
